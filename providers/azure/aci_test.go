...
func TestCreatePodWithGPU(t *testing.T) {
	...
	aciServerMocker.OnCreate = func(subscription, resourceGroup, containerGroup string, cg *aci.ContainerGroup) (int, interface{}) {
		...
		assert.Equal(t, 1.98, cg.ContainerGroupProperties.Containers[0].Resources.Requests.CPU, "Request CPU is not expected")
		assert.Equal(t, 3.4, cg.ContainerGroupProperties.Containers[0].Resources.Requests.MemoryInGB, "Request Memory is not expected")
		assert.NotNil(t, cg.ContainerGroupProperties.Containers[0].Resources.Requests.GPU, "Requests GPU is not expected")
		assert.Equal(t, int32(10), cg.ContainerGroupProperties.Containers[0].Resources.Requests.GPU.Count, "Requests GPU Count is not expected")
		assert.Equal(t, gpuSKU, cg.ContainerGroupProperties.Containers[0].Resources.Requests.GPU.SKU, "Requests GPU SKU is not expected")
		assert.NotNil(t, cg.ContainerGroupProperties.Containers[0].Resources.Limits.GPU, "Limits GPU is not expected")
		assert.Equal(t, int32(10), cg.ContainerGroupProperties.Containers[0].Resources.Limits.GPU.Count, "Requests GPU Count is not expected")
		assert.Equal(t, gpuSKU, cg.ContainerGroupProperties.Containers[0].Resources.Limits.GPU.SKU, "Requests GPU SKU is not expected")
...
func TestCreatePodWithGPUSKU(t *testing.T) {
	...
	aciServerMocker.OnCreate = func(subscription, resourceGroup, containerGroup string, cg *aci.ContainerGroup) (int, interface{}) {
		...
		assert.Equal(t, 3.4, cg.ContainerGroupProperties.Containers[0].Resources.Requests.MemoryInGB, "Request Memory is not expected")
		assert.NotNil(t, cg.ContainerGroupProperties.Containers[0].Resources.Requests.GPU, "Requests GPU is not expected")
		assert.Equal(t, int32(1), cg.ContainerGroupProperties.Containers[0].Resources.Requests.GPU.Count, "Requests GPU Count is not expected")
		assert.Equal(t, gpuSKU, cg.ContainerGroupProperties.Containers[0].Resources.Requests.GPU.SKU, "Requests GPU SKU is not expected")
		assert.NotNil(t, cg.ContainerGroupProperties.Containers[0].Resources.Limits.GPU, "Limits GPU is not expected")
		assert.Equal(t, int32(1), cg.ContainerGroupProperties.Containers[0].Resources.Limits.GPU.Count, "Requests GPU Count is not expected")
		assert.Equal(t, gpuSKU, cg.ContainerGroupProperties.Containers[0].Resources.Limits.GPU.SKU, "Requests GPU SKU is not expected")
...
func TestGetPodWithGPU(t *testing.T) {
	...
	assert.NotNil(t, pod.Spec.Containers, "Containers should not be nil")
	assert.NotNil(t, pod.Spec.Containers[0].Resources, "Containers[0].Resources should not be nil")
	assert.NotNil(t, pod.Spec.Containers[0].Resources.Requests, "Containers[0].Resources.Requests should not be nil")
	assert.Equal(
		t,
		ptrQuantity(resource.MustParse("0.99")).Value(),
		pod.Spec.Containers[0].Resources.Requests.Cpu().Value(),
	...
	assert.Equal(
		t,
		ptrQuantity(resource.MustParse("5")).Value(),
		ptrQuantity(gpuQuantity).Value(),
		"Containers[0].Resources.Requests.GPU.Count doesn't match")
	assert.NotNil(t, pod.Spec.Containers[0].Resources.Limits, "Containers[0].Resources.Limits should not be nil")
