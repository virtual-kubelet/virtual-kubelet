...
func makeEnvironmentMap(ctx context.Context, pod *corev1.Pod, container *corev1.Container, rm *manager.ResourceManager, recorder record.EventRecorder, res map[string]string) error {
	...
	for _, env := range container.Env {
		if env.ValueFrom != nil {
			if val, err := getEnvValueFromValueFrom(ctx, &env, pod, rm, recorder); err != nil {
				return err
			} else if val != nil {
				res[env.Name] = *val
			}
			// The other case here is where the value was optional, and we could not resolve it. In that case, do not set
			// the environment variable at all. The underlying function called from getEnvValueFromValueFrom should record
			// and event indicating the environment variable was unable to be resolved, and potentially log an error
		} else {
...
func getEnvValueFromValueFrom(ctx context.Context, env *corev1.EnvVar, pod *corev1.Pod, rm *manager.ResourceManager, recorder record.EventRecorder) (*string, error) {
	switch {
	case env.ValueFrom.ConfigMapKeyRef != nil:
		return getEnvValueFromConfigMapKeyRef(ctx, env.Name, env.ValueFrom.ConfigMapKeyRef, pod, rm, recorder)
	case env.ValueFrom.SecretKeyRef != nil:
		return getEnvValueFromSecretKeyRef(ctx, env.Name, env.ValueFrom.SecretKeyRef, pod, rm, recorder)
	case env.ValueFrom.FieldRef != nil:
		return getEnvValueFromFieldRef(ctx, env.Name, env.ValueFrom.FieldRef, pod, rm, recorder)
	case env.ValueFrom.ResourceFieldRef != nil:
		return getEnvValueFromResourceFieldRef(ctx, env.Name, env.ValueFrom.ResourceFieldRef, pod, rm, recorder)
	default:
		// This should never happen.
		// See: https://github.com/kubernetes/kubernetes/blob/5935fcd704fe89048776d02cf1ef4f939743c042/pkg/apis/core/validation/validation.go#L2171-L2208
		// if value from is not nil, it must be one of the above.
		return nil, fmt.Errorf("key %q sources value from unknown field: %v", env.Name, *env.ValueFrom)
	}
}

func getEnvValueFromConfigMapKeyRef(ctx context.Context, name string, vf *corev1.ConfigMapKeySelector, pod *corev1.Pod, rm *manager.ResourceManager, recorder record.EventRecorder) (*string, error) {
	// Check whether the key reference is optional.
	// This will control whether we fail when unable to read the requested key.
	optional := pointer.BoolPtrDerefOr(vf.Optional, false)
	// Try to grab the referenced configmap.
	...
	if err != nil {
		...
		if optional {
			if errors.IsNotFound(err) {
				recorder.Eventf(pod, corev1.EventTypeWarning, ReasonOptionalConfigMapNotFound, "skipping optional envvar %q: configmap %q not found", name, vf.Name)
			} else {
				log.G(ctx).Warnf("failed to read configmap %q: %v", vf.Name, err)
				recorder.Eventf(pod, corev1.EventTypeWarning, ReasonFailedToReadOptionalConfigMap, "skipping optional envvar %q: failed to read configmap %q", name, vf.Name)
			}
			// Continue on to the next reference.
			return nil, nil
		}
		...
		return nil, fmt.Errorf("failed to read configmap %q: %w", vf.Name, err)
	}

	// At this point we have successfully fetched the target configmap.
	// We must now try to grab the requested key.
	keyValue, keyExists := m.Data[vf.Key]
	if !keyExists {
		// The requested key does not exist.
		// However, we should not fail if the key reference is optional.
		if optional {
			// Continue on to the next reference.
			recorder.Eventf(pod, corev1.EventTypeWarning, ReasonOptionalConfigMapKeyNotFound, "skipping optional envvar %q: key %q does not exist in configmap %q", name, vf.Key, vf.Name)
			return nil, nil
		}
		// At this point we know the key reference is mandatory.
...
func getEnvValueFromSecretKeyRef(ctx context.Context, name string, vf *corev1.SecretKeySelector, pod *corev1.Pod, rm *manager.ResourceManager, recorder record.EventRecorder) (*string, error) {
	// Check whether the key reference is optional.
	// This will control whether we fail when unable to read the requested key.
	optional := pointer.BoolPtrDerefOr(vf.Optional, false)
	// Try to grab the referenced secret.
	...
	if err != nil {
		...
		if optional {
			if errors.IsNotFound(err) {
				recorder.Eventf(pod, corev1.EventTypeWarning, ReasonOptionalSecretNotFound, "skipping optional envvar %q: secret %q not found", name, vf.Name)
			} else {
				log.G(ctx).WithError(err).Warnf("failed to read secret %q", vf.Name)
				recorder.Eventf(pod, corev1.EventTypeWarning, ReasonFailedToReadOptionalSecret, "skipping optional envvar %q: failed to read secret %q", name, vf.Name)
			}
			return nil, nil
		}
		// At this point we know the key reference is mandatory.
		...
		return nil, fmt.Errorf("failed to read secret %q: %w", vf.Name, err)
	}
	// At this point we have successfully fetched the target secret.
	// We must now try to grab the requested key.
	keyValue, keyExists := s.Data[vf.Key]
	if !keyExists {
		// The requested key does not exist.
		// However, we should not fail if the key reference is optional.
		if optional {
			// Continue on to the next reference.
			recorder.Eventf(pod, corev1.EventTypeWarning, ReasonOptionalSecretKeyNotFound, "skipping optional envvar %q: key %q does not exist in secret %q", name, vf.Key, vf.Name)
			return nil, nil
		}
		// At this point we know the key reference is mandatory.
		// Hence, we should fail.
...
func getEnvValueFromFieldRef(ctx context.Context, name string, vf *corev1.ObjectFieldSelector, pod *corev1.Pod, rm *manager.ResourceManager, recorder record.EventRecorder) (*string, error) {
...
func getEnvValueFromResourceFieldRef(ctx context.Context, name string, vf *corev1.ResourceFieldSelector, pod *corev1.Pod, rm *manager.ResourceManager, recorder record.EventRecorder) (*string, error) {
