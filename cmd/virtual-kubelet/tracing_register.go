...
import (
	"github.com/virtual-kubelet/virtual-kubelet/node/cli/opencensus"
)
...
func AvailableTraceExporters() map[string]opencensus.ExporterInitFunc {
	out := make(map[string]opencensus.ExporterInitFunc, len(tracingExporters))
	for k, v := range tracingExporters {
		out[k] = v
	}
	return out
}
